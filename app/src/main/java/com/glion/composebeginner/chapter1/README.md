...
- verticalArrangement 종류
  - ![verticalArrangement 종류](http://glion.iptime.org/Blog/250211_compose1/verticalArrangement.gif)
...
- horizontalArrangement 종류
  - ![horizontalArrangement](http://glion.iptime.org/Blog/250211_compose1/horizontalArrangement.gif)
...
- Composable 이나 레이아웃 요소(Column, Row, Box 등) 에서 매개변수로 전달되어 각종 설정을 할 수 있음(모든 Composable 에 적용이 가능하다)
  - ### 패딩 지정
    ...
    - ```.dp``` 는 ```androidx.compose.ui.unit``` 내에 확장함수로 지정되어 있으며 Dp 타입의 객체로 변환된다. 이는 애플리케이션 실행 시 디바이스의 밀도에 맞춰 실제 px 값으로 변환되어 적용된다.<br>(기존 View 방식에서 16으로 지정했을 경우 px로 적용되는데 반해 간략하게 16.dp 로 지정하면 Composable 에서는 dp 로 적용된다.)
    - ```
  ...
  - ### 사이즈 지정
    ...
    - ```Modifier.fillMaxSize()``` : 요소의 크기를 화면 전체에 맞게 적용한다.
      ...
      - <figure class="half" align="center"> <a href="link"><img src="http://glion.iptime.org/Blog/250211_compose1/fillMaxSize10.jpg" width="20%" alt="fillMaxSize()"></a>  <a href="link"><img src="http://glion.iptime.org/Blog/250211_compose1/fillMaxSize05.jpg" width="20%" alt="fillMaxSize(0.5f)"></a>  <figcaption>fillMaxSize() vs fillMaxSize(0.5f).</figcaption></figure>
    ...
    - ```Modifier.weight()``` : 가중치를 주어 요소가 차지할 영역을 지정한다.(view 방식에서 width/height 를 0으로 주고, weight 를 지정하는 것과 동일한 효과)
      - 반드시 자식 요소에만 적용이 가능하다. @Composable 함수의 최상위 요소에는 적용이 불가능하다
        ```
        @Composable
        fun ModifierWeightTest() {
        Column(
        modifier = Modifier.weight(1f) // 불가능
        ) {
            ...
          }
        }
        ...
        fun ModifierWeightTest() {
            Column() {
                Column(
                    modifier = Modifier.weight(1f) // 가능
                ) {
...
- Image Composable 의 속성
  - ```alpha``` : 이미지 투명도 조절
  - ```colorFilter``` : 이미지 색상 변경. 매개변수로 변경할 색상과 blendMode 를 넣을 수 있다.<br>xml ImageView 의 tint 와 동일한 효과이며, blendMode 는 tintMode 와 같다.
    ```
      Image(
          painter = painterResource(id = R.drawable.drawbleId),
